...
```{r}
commonAnimals <- intersect(DF1, DF2)
...
```{r}
estan_en_1 <- setdiff(rownames(DF1), rownames(DF2))
estan_en_1

estan_en_2 <- setdiff(rownames(DF2), rownames(DF1))
estan_en_2
...
lm(formula = VitC ~ HeadWt, data = cabbages, subset = which(Cult == "c39"))
lm(formula = VitC ~ HeadWt, data = cabbages, subset = which(Cult == "c52"))
